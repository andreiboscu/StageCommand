...
public class SoundWindow implements MessageListener {
    ...
    public static void showUI(MessageBoard board) {
        SwingUtilities.invokeLater(() -> new SoundWindow(board));
    }

    public SoundWindow(MessageBoard board) {
        this.board = board;
...
//    
    ...
    public void onMessageReceived(String msg) {
        SwingUtilities.invokeLater(() -> {
            ...
            // Verifică dacă mesajul este pentru SOUND
            if (log.startsWith("TO: SOUND") || log.startsWith("TO: ALL STATIONS")) {
    ...
    private void init() {
        frame = new JFrame("Sound Station");
        ...
        frame.addWindowListener(new WindowAdapter() {
            public void windowClosing(WindowEvent e) {
                board.removeMessageListener(SoundWindow.this);
            }
        });
        ...
        JLabel title = new JLabel("Sound Station", SwingConstants.CENTER);
        ...
        current.setBorder(new CompoundBorder(
                new LineBorder(new Color(100,180,255), 2),
                new EmptyBorder(25,25,25,25)
        ));

        JLabel prio = new JLabel("PRIORITY: HIGH");
        prio.setForeground(new Color(100,180,255));
        ...
        // FORȚĂM DIMENSIUNEA LOG-ULUI (mai înalt)
